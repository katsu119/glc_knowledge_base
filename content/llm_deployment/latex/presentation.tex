...
% ---------- Twemoji + LaTeX 命令符号方案 ----------
\usepackage{twemojis}
% 以下 Unicode 字符已全部替换为 LaTeX 命令 / twemoji：
%   U+00A5 ¥  -> \textyen{}         U+2014 -- -> ---
%   U+2192 -> -> $\rightarrow$       U+2194 <-> -> $\leftrightarrow$
%   U+25CF ●  -> \twemoji{black circle}
%   U+2605 ★  -> \twemoji{glowing star}
%   U+2606 ☆  -> $\star$
%   U+2713 ✓  -> \twemoji{check mark}
%   U+2717 ✗  -> \twemoji{cross mark}
%   U+26A0 ⚠ -> \twemoji{warning}
...
\begin{frame}{PoC 三个层次}
  \begin{columns}[T]
    ...
    \column{0.32\textwidth}
    \begin{alertblock}{层次二：低成本 PoC（\textasciitilde \textyen{}15K, 1--2 天）}
      \textbf{甜点方案！}
      \begin{itemize}
    ...
    \column{0.32\textwidth}
    \begin{block}{层次三：准生产 PoC（\textasciitilde \textyen{}60K--\textyen{}120K）}
      \begin{itemize}
...
\begin{frame}{Mac Mini 是否够？}
  \begin{alertblock}{坦白说：对于企业 PoC，不推荐 Mac Mini}
    \begin{itemize}
      \item M4 Pro Mac mini 64GB 统一内存（约 \textyen{}20K+）------不如二手 RTX 4090 工作站
...
\begin{frame}{PoC 方案成本对比}
  ...
  \begin{table}
    ...
    \begin{tabular}{p{4.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.8cm}p{1.5cm}}
      ...
      \midrule
      MacBook Pro M4 Pro 24GB       & \textyen{}18K     & 30W             & \textyen{}17             & 7B Q4           & 15--20 t/s    \\
      \rowcolor{green!15}
      RTX 4090 工作站               & \textyen{}25K             & 500W            & \textyen{}288            & 32B Q4          & 30--50 t/s    \\
      2$\times$RTX 4090              & \textyen{}40K             & 800W            & \textyen{}460            & 70B Q4          & 15--25 t/s    \\
      A100 80GB 二手               & \textyen{}50K             & 400W            & \textyen{}230            & 70B FP16        & 40--60 t/s    \\
      \bottomrule
    \end{tabular}
  \end{table}
  ...
  \vspace{0.3em}
  \begin{keypoint}
    自建 RTX 4090 方案在 3 个月内回本（对比云 GPU 月费 \textyen{}5K--\textyen{}8K）。
  \end{keypoint}
\end{frame}
...
\begin{frame}{IC 设计流程 LLM 介入全景}
  ...
  \begin{itemize}
    \item \textcolor{darkgreen}{\twemoji{black circle} 绿色(G)} = 高收益安全区：脚本/RAG/DevOps
    \item \textcolor{gold}{\twemoji{black circle} 黄色(Y)} = 中收益需审查：RTL 骨架/报告解读
    \item \textcolor{darkred}{\twemoji{black circle} 红色(R)} = 高风险不能替代：SPICE/综合/STA 关键路径
  \end{itemize}
\end{frame}

% -------------------- 模拟 IC --------------------
\begin{frame}{模拟 IC 设计：电路与架构 \twemoji{glowing star}\twemoji{glowing star}$\star$$\star$$\star$}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \begin{block}{能做什么 \twemoji{check mark}}
      \begin{itemize}
        \item 读网表，解释电路拓扑
    ...
    \column{0.5\textwidth}
    \begin{alertblock}{不能做什么 \twemoji{cross mark}（危险区域）}
      \begin{itemize}
...
\begin{frame}{模拟 IC 设计：脚本与工具链 \twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}}
...
% -------------------- 数字 IC --------------------
\begin{frame}{数字 IC 设计：RTL 代码 \twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}$\star$$\star$}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \begin{block}{能做什么 \twemoji{check mark}}
      \begin{itemize}
    ...
    \column{0.5\textwidth}
    \begin{alertblock}{不能做什么 \twemoji{cross mark}}
      \begin{itemize}
...
\begin{frame}{数字 IC 设计：UVM 验证 \twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}$\star$}
...
\begin{frame}{软件/基础设施场景}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \begin{block}{Linux 运维与 DevOps \twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}}
      \begin{itemize}
    ...
    \column{0.5\textwidth}
    \begin{block}{仿真任务调度 \twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}\twemoji{glowing star}$\star$}
      \begin{itemize}
...
\begin{frame}{Embedding 与 Chunking 策略}
  \begin{columns}[T]
    ...
    \begin{block}{Embedding：语义映射}
      \begin{itemize}
        \item 「时钟域交叉」$\leftrightarrow$「CDC」$\rightarrow$ 相近向量
        \item 「Bandgap 温度系数」$\leftrightarrow$「TC」$\rightarrow$ 相近向量
...
\begin{frame}{什么是 AI Agent？}
  ...
  \begin{enumerate}
    \item 用户目标 $\rightarrow$ Agent 规划步骤 $\rightarrow$ 调用工具 $\rightarrow$ 观察结果 $\rightarrow$ 调整计划 $\rightarrow$ 循环至完成
  \end{enumerate}
\end{frame}
...
\begin{frame}{EDA 场景的 Agent 安全边界}
  ...
  \begin{itemize}
    \item \textcolor{darkgreen}{\twemoji{black circle} 绿圈} = 只读自动化（安全）：读 log、读报告、搜索文档
    \item \textcolor{gold}{\twemoji{black circle} 黄圈} = 写入需人工审核：修改 constraint/RTL/提交代码
    \item \textcolor{darkred}{\twemoji{black circle} 红圈} = 禁止自动化：自动修改 tapeout 数据
  \end{itemize}
...
% -------------------- 方案 A --------------------
\begin{frame}{方案 A：最低成本 PoC（\textyen{}25K--\textyen{}35K）}
  \small
  \begin{columns}[T]
    \column{0.45\textwidth}
    \begin{block}{硬件配置}
      \begin{itemize}
        \item \textbf{GPU}：1$\times$RTX 4090 24GB（约 \textyen{}13K）
...
% -------------------- 方案 B --------------------
\begin{frame}{方案 B：初创公司推荐方案（\textyen{}60K--\textyen{}100K）}
  \small
  \begin{columns}[T]
    \column{0.45\textwidth}
    \begin{block}{硬件配置}
...
\begin{frame}{关键决策：买硬件 vs 用云}
  \begin{table}
    ...
    \begin{tabular}{p{3cm}p{3.5cm}p{4.5cm}}
      ...
      \midrule
      硬件成本           & \textyen{}13K（一次性）              & \textyen{}0                         \\
      月费               & \textyen{}300（电费）                & \textyen{}5K--\textyen{}8K（24/7）          \\
      回本周期           & ---                           & \textbf{2--3 个月}          \\
...
\begin{frame}{最后的话：行动起来}
  \begin{enumerate}
    \item \textbf{先花 \textyen{}25K 做 PoC}。买 RTX 4090，装 Ubuntu，部署 vLLM + Qwen2.5-Coder-32B + Open WebUI。3--5 个工程师试用一个月。

    \item \textbf{反馈正面} $\rightarrow$ 升级方案 B（双卡或 RTX 5090），建设 RAG 知识库。

    \item \textbf{反馈负面} $\rightarrow$ \textyen{}25K 显卡给工程师当深度学习工作站，不会浪费。
