...
\node[dtpanel, fill=blue!3, draw=blue!30, anchor=north west] (dt-addrspace) at (19cm, -2cm) {
  \textbf{\normalsize Process Address Space Layout \quad 进程地址空间布局}
  \\[4pt]
  A Linux process's virtual memory is divided into several regions, managed by \texttt{task\_struct$\rightarrow$mm} (\textbf{mm\_struct}):
  \\[6pt]
  \begin{minipage}{21.5cm}
  \centering
  \begin{tikzpicture}[font=\scriptsize]
    % --- Memory Bar (left side) ---
    \def\barx{0.8}
    \def\barw{2.0}
    \def\barheight{9.0}
    ...
    \draw[draw=black!40] (\barx,0) rectangle (\barx+\barw,\barheight);

    % Kernel space
    \fill[red!10] (\barx,6.8) rectangle (\barx+\barw,\barheight);
    \draw[draw=red!30, thick] (\barx,6.8) rectangle (\barx+\barw,\barheight);
    \node[rotate=90, font=\small, red!70!black] at (\barx+1.0,8.0) {内核空间 (Kernel Space)};

    % User space boundary
    \draw[dashed, gray!60] (\barx,6.8) -- (\barx+\barw+1.0,6.8);
    \node[right, font=\tiny, gray!60] at (\barx+\barw+0.1,6.8) {TASK\_SIZE (0x00007FFFFFFFFFFF)};

    % Stack
    \fill[yellow!15] (\barx,5.8) rectangle (\barx+\barw,6.8);
    \draw[draw=orange!50!black!50, thick] (\barx,5.8) rectangle (\barx+\barw,6.8);
    \draw[<->, thick, >=Stealth, orange!70!black] (\barx+\barw+0.3,5.8) -- (\barx+\barw+0.3,6.8);
    \node[right, font=\small, orange!70!black] at (\barx+\barw+0.5,6.3) {Stack (向下增长)};
    \node[right, font=\tiny] at (\barx+\barw+0.5,5.8) {0x7FFF...};

    % mmap region
    \fill[teal!12] (\barx,4.0) rectangle (\barx+\barw,5.8);
    \draw[draw=teal!50!black!60, thick] (\barx,4.0) rectangle (\barx+\barw,5.8);
    \node[rotate=90, font=\small, teal!70!black] at (\barx+1.0,4.9) {mmap 区域 (文件映射/匿名映射/共享库)};

    % Heap
    \fill[green!15] (\barx,2.0) rectangle (\barx+\barw,4.0);
    \draw[draw=green!50!black!50, thick] (\barx,2.0) rectangle (\barx+\barw,4.0);
    \draw[<->, thick, >=Stealth, green!70!black] (\barx+\barw+0.3,2.0) -- (\barx+\barw+0.3,4.0);
    \node[right, font=\small, green!70!black] at (\barx+\barw+0.5,3.0) {Heap (向上增长, brk/sbrk)};

    % BSS
    \fill[orange!15] (\barx,1.2) rectangle (\barx+\barw,2.0);
    \draw[draw=orange!50!black!50, thick] (\barx,1.2) rectangle (\barx+\barw,2.0);
    \node[font=\small] at (\barx+1.0,1.6) {.bss (未初始化数据)};

    % Data
    \fill[blue!15] (\barx,0.4) rectangle (\barx+\barw,1.2);
    \draw[draw=blue!50!black!50, thick] (\barx,0.4) rectangle (\barx+\barw,1.2);
    \node[font=\small] at (\barx+1.0,0.8) {.data (初始化数据)};

    % Text
    \fill[purple!15] (\barx,0) rectangle (\barx+\barw,0.4);
    \draw[draw=purple!50!black!50, thick] (\barx,0) rectangle (\barx+\barw,0.4);
    \node[font=\small] at (\barx+1.0,0.2) {.text (代码)};

    % Labels on left side of bar
    \node[right, font=\tiny, gray!70] at (0, 9.0) {0xFFFF...FFFF};
    \node[right, font=\tiny, gray!70] at (0, 6.8) {0x0000...0000};

    % --- Data Structure Detail (right side) ---
    \begin{scope}[shift={(\barx+\barw+2.0, 6.5)}]
      % mm_struct block
      \node[dtblock, fill=blue!8, draw=blue!40, minimum width=6cm, minimum height=1.8cm, align=center] (mm) at (4, 0) {
        \textbf{mm\_struct} \qquad 进程内存描述符
      };
      \node[dtsmall, fill=blue!5, draw=blue!20, below=0.15cm of mm.south, anchor=north, align=center] (mm-fields) {
        \texttt{pgd} (页全局目录) \quad | \quad \texttt{mmap} (VMA 链表) \quad | \quad \texttt{mm\_rb} (VMA 红黑树)
      };
      \node[dtsmall, fill=blue!5, draw=blue!20, below=0.08cm of mm-fields.south, anchor=north, align=center] (mm-fields2) {
        \texttt{brk} (堆顶) \quad | \quad \texttt{start\_stack} / \texttt{start\_brk} / \texttt{start\_mmap} \quad | \quad \texttt{rss\_stat}
      };

      % VMA detail
      \node[dtblock, fill=teal!8, draw=teal!40, minimum width=6cm, align=center] (vma) at (4, -2.2) {
        \textbf{vm\_area\_struct} \qquad 虚拟内存区域 (VMA)
      };
      \node[dtsmall, fill=teal!5, draw=teal!20, below=0.15cm of vma.south, anchor=north, align=center] (vma-fields) {
        \texttt{vm\_start} / \texttt{vm\_end} \quad | \quad \texttt{vm\_flags} (VM\_READ/WRITE/EXEC/SHARED)
      };
      \node[dtsmall, fill=teal!5, draw=teal!20, below=0.08cm of vma-fields.south, anchor=north, align=center] (vma-fields2) {
        \texttt{vm\_file} (映射文件) \quad | \quad \texttt{vm\_ops} (open/close/fault) \quad | \quad \texttt{vm\_rb} (红黑树节点)
      };

      % Connection between mm_struct and VMA
      \draw[arrow, blue!50!black] (mm.south) -- node[right, font=\tiny, align=left] {mmap 指向\\VMA 链表头} (vma.north);

      % grow direction annotations
      \node[font=\tiny, align=left, blue!60!black] at (0, -3.8) {
        \textbf{页表:} \texttt{pgd} 指向进程的 4 级/5 级页表\\
        \textbf{缺页:} 访问未映射地址 → \#PF → do\_page\_fault\\
        \textbf{brk:} \texttt{sbrk()} / \texttt{brk()} 系统调用调整堆边界\\
        \textbf{COW:} fork 后父子共享页, 写时复制
      };
    \end{scope}

    % --- Flow arrows from bar to structures ---
    \draw[->, dashed, >=Stealth, gray!50] (5.0, 6.0) -- (7.0, 6.5);
    \node[font=\tiny, gray!60] at (6.0, 5.7) {每个区域对应一个 VMA};
...
% ============================================================
\node[dtpanel, fill=purple!3, draw=purple!30, below=0.3cm of dt-addrspace.south west, anchor=north west] (dt-syscall) {
...
% ============================================================
\node[dtpanel, fill=green!3, draw=green!40, below=0.3cm of dt-syscall.south west, anchor=north west] (dt-proc) {
...
% ============================================================
\node[dtpanel, fill=red!3, draw=red!40, below=0.3cm of dt-proc.south west, anchor=north west] (dt-mem) {
...
% ============================================================
\node[dtpanel, fill=orange!3, draw=orange!40, below=0.3cm of dt-mem.south west, anchor=north west] (dt-vfs) {
...
% ============================================================
\node[dtpanel, fill=cyan!3, draw=cyan!40, below=0.3cm of dt-vfs.south west, anchor=north west] (dt-net) {
...
% ============================================================
\node[dtpanel, fill=olive!3, draw=olive!50!black!40, below=0.3cm of dt-net.south west, anchor=north west] (dt-drv) {
...
% ============================================================
\node[dtpanel, fill=yellow!3, draw=yellow!50!black!50, below=0.3cm of dt-drv.south west, anchor=north west] (dt-ipc) {
...
% ============================================================
\node[dtpanel, fill=magenta!3, draw=magenta!40, below=0.3cm of dt-ipc.south west, anchor=north west] (dt-intr) {
...
% ============================================================
\node[dtpanel, fill=brown!3, draw=brown!50!black!50, below=0.3cm of dt-intr.south west, anchor=north west] (dt-sync) {
...
% ============================================================
\node[dtpanel, fill=gray!3, draw=gray!50, below=0.3cm of dt-sync.south west, anchor=north west] (dt-time-arch) {
...
% ============================================================
\node[dtpanel, fill=black!2, draw=black!40, below=0.3cm of dt-time-arch.south west, anchor=north west] (dt-hw) {
